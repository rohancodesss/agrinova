...
- Auto-irrigation: when `auto_mist` is on and soil reads DRY (<40 %) → spray when soil % < `AUTO_MIST_START_PCT` (30), stop when ≥ `AUTO_MIST_WET_PCT` (40) — correct direction for a working sensor (raw high=dry); 10 min safety cap, max 4/h. Schedules fire daily via `scheduler_loop`; daily summary at `DAILY_SUMMARY_TIME` (20:00).
