...
- i18n: `MSGS` dict + `msg(key, **kw)`; `settings["lang"]` ∈ en/hi/te/te_en. `/lang` shows a 3-button picker; `/hindi`, `/telugu`, `/english` are aliases. Translated: intruder, lockdown, mist, soil line, climate, auto-mist, camera-removed, rain-skip. Everything else stays English.
...
- Weather: `fetch_weather()` hits `https://wttr.in/<location>?format=j1` (no key, 15 min cache). `rain_expected()` → skip SCHEDULED sprays only (auto-mist ignores weather, follows the soil sensor) when chance ≥ 60 % in next 6 h (`settings["rain_skip"]`, default on). `/weather`, `/weather set <city>`, `/rain_skip on|off`. Location persisted in `settings["location"]`.
- Auto-irrigation: when `auto_mist` is on and soil reads DRY (<40 %) → spray when soil % < `AUTO_MIST_START_PCT` (30), stop when ≥ `AUTO_MIST_WET_PCT` (40) — correct direction for a working sensor (raw high=dry); 10 min safety cap. Schedules fire daily via `scheduler_loop`; daily summary at `DAILY_SUMMARY_TIME` (20:00).
- `/setup` wizard: 5 button-driven questions (lang, lockdown, auto_mist, rain_skip, daily_msgs) via `SETUP_STEPS`/`setup_idx`; callbacks `/setup pick <val>`, Skip keeps current, Cancel aborts; summary at end. `/start` runs it on first contact (no state file), else /help. Aliases /wizard /settings.
- Message budget: `settings["daily_msgs"]` (default 30, 0=∞), `/messages [n]` with button picker. `send_routine(text, force)` gates ROUTINE messages (soil, climate, auto-mist, rain-skip): daily cap + even spacing (86400/limit; `force` skips spacing only). CRITICAL (intruder, camera-removed, disconnects, bridge on/off) and command replies bypass it. Soil updates are change-driven: sent only if WET/DRY flipped or Δmoisture≥3 / Δtemp≥1.5 / Δhumidity≥8 vs last SENT reading.
- Alerts: soil on meaningful change (budgeted); climate warning (≥35 °C or ≤30 % RH) every 30 min; FPGA/Arduino/camera disconnect + reconnect; bridge-online on start; camera-removal CRITICAL.
...
- Camera: `request_capture` uses the Windows VM when its client polls (`camera_online()`), else captures on the Mac itself — `detect_local_camera()`/`local_capture()` via ffmpeg avfoundation (C170 preferred, cv2-era note: first capture triggers macOS camera permission for the user's terminal). Heartbeat/status report which camera is active. Relay details: `request_capture(kind, duration, timeout)`; Windows POSTs bytes back; header `X-Camera-Status: removed` → CRITICAL alert. `camera_online()` = polled within 60 s.
